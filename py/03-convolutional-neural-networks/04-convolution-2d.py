...
class Convolution2D(Layer):
    ...
    def forward(self, x: Tensor):
        ...
        def backward_fn():
            if self.weight.requires_grad:
                self.weight.grad = p.grad.reshape(-1, self.out_channels).T.dot(
                    (patches.reshape(-1, self.kernel_size ** 2)))
            if self.bias.requires_grad:
                self.bias.grad = np.sum(p.grad.reshape(-1, self.out_channels), axis=0)
...
LEARNING_RATE = 0.1
EPOCHES = 10
BATCHES, CHANNELS, KERNELS = (2, 16, 3)
...
hidden = Linear(convolved_row * convolved_column * CHANNELS, 64)
output = Linear(64, dataset.label_size())
model = Sequential([kernel, Flatten(), Dropout(), hidden, Tanh(), output, Softmax()])
